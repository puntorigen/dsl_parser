  ...
  class dsl_parser {
    ...
    */
    ...
    addNode() {
      ...
      return _asyncToGenerator(function* () {
        ...
        //grab parentID
        var parent = yield _this3.$('node[ID=' + parent_id + ']').each( /*#__PURE__*/function () {
          var _ref2 = _asyncToGenerator(function* (i, elem) {
            var cur = me.$(elem);
            var txml = yield this.nodeToXML(node);
            cur.append(txml);
          });
    ...
    nodeToXML() {
      ...
      return _asyncToGenerator(function* () {
        var {
          node = _this4.throwIfMissing('node - nodeToXML')
        ...
        var nodeToObj = function nodeToObj(node) {
          ...
          if (node_obj.text != '') obj.NODE['@TEXT'] = node_obj.text; //attributes
    ...
    */
    ...
    getNode() {
      ...
      return _asyncToGenerator(function* () {
        var {
          id = _this5.throwIfMissing('id - getNode'),
          recurse = true,
          justlevel,
          dates = true,
          $ = false,
        ...
        if (_this5.$ === null) throw new Error('call process() first!');

        if (id in _this5.x_memory_cache.getNode && _this5.x_memory_cache.getNode[id].valid && _this5.x_memory_cache.getNode[id].valid == true && nodes_raw == false && $ == false) {
          return _this5.x_memory_cache.getNode[id];
        } else {
          var _me = _this5;
          var resp = {
            level: -1,
          ...
          var nodes = yield _me.$('node[ID=' + id + ']').each( /*#__PURE__*/function () {
            var _ref3 = _asyncToGenerator(function* (i, elem) {
              ...
              if (hash_content && hash_content == true) {
                ...
                resp.hash_content = yield _me.hash(content_); //me.debug.outT({ message:'hash created:'+resp.hash_content });
              } //
              ...
              cur.find('node[ID=' + resp.id + '] > attribute').map(function (a, a_elem) {
                var _fila = _me.$(a_elem); //tmp_fila[_fila.attr('NAME')] = _fila.attr('VALUE');
              ...
              cur.find('node[ID=' + resp.id + '] > icon').map(function (a, a_elem) {
                resp.icons.push(_me.$(a_elem).attr('BUILTIN'));
              }); // fonts definition

              cur.find('node[ID=' + resp.id + '] > font').map(function (a, a_elem) {
                var _fila = _me.$(a_elem);
              ...
              cur.find('node[ID=' + resp.id + '] > richcontent[TYPE=NODE] body').map(function (a, a_elem) {
                resp.text_rich = _me.$(a_elem).html();

                _me.$(a_elem).find('img[src]').map(function (i, i_elem) {
                  resp.image = _me.$(i_elem).attr('src');
                });
              }); // get notes on node if any

              cur.find('node[ID=' + resp.id + '] > richcontent[TYPE=NOTE] body').map(function (a, a_elem) {
                resp.text_note = _me.$(a_elem).text();
                resp.text_note_html = _me.$(a_elem).html();
          ...
          if (resp.valid && resp.valid == true && nodes_raw == false && $ == false) {
            ...
            _this5.x_memory_cache.getNode[id] = resp;
          } // reply
    ...
    */
    ...
    getParentNodesIDs() {
      ...
      return _asyncToGenerator(function* () {
        var {
          id = _this7.throwIfMissing('id - ParentNodesIDs'),
          array = false
        } = _arguments6.length > 0 && _arguments6[0] !== undefined ? _arguments6[0] : {};

        var padres = _this7.$("node[ID=".concat(id, "]")).parents('node');
    ...
    */
    ...
    getChildrenNodesIDs() {
      ...
      return _asyncToGenerator(function* () {
        var {
          id = _this8.throwIfMissing('id - ChildrenNodesIDs'),
          array = false
        } = _arguments7.length > 0 && _arguments7[0] !== undefined ? _arguments7[0] : {};

        var hijos = _this8.$("node[ID=".concat(id, "]")).find('node');
    ...
    */
    ...
    getBrotherNodesIDs() {
      ...
      return _asyncToGenerator(function* () {
        var {
          id = _this9.throwIfMissing('id - BrotherNodesIDs'),
          before = true,
          after = true,
          array = false
        } = _arguments8.length > 0 && _arguments8[0] !== undefined ? _arguments8[0] : {};
        var me_data = yield _this9.getNode({
          id: id,
          recurse: false,
          $: true
        });
    ...
    */
    ...
    createGitVersion(extrastep) {
      var _this10 = this;

      return _asyncToGenerator(function* () {
        // 1) get copy of current DSL content into memory (for restoring after returning)
        var copy = _this10.$.html(),
            me = _this10; // 2) get all nodes


        var nodes = _this10.$("node"); // 
        ...
        var resp = _this10.$.html(); // 8) extrastep


        if (extrastep && typeof extrastep == 'function') {
          resp = extrastep(_this10.$);
        }

        resp = _this10.help.fixAccents(resp, true); // recover original tags to current parser $.

        _this10.$ = _this10.$.load(copy, {
          ignoreWhitespace: false,
          xmlMode: true,
          decodeEntities: false
        }); // return
